...
public class MailSearchServiceimpl implements MailSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //2.执行检索请求
            SearchResponse response = esRestClient.search(searchRequest, GulimailElasticSearchConfig.COMMON_OPTIONS);
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        //5、分页信息-页码
        result.setPageNume(param.getPageNum());
        //5、1分页信息、总记录数
    ...
    private SearchRequest buildSearchRequest(SearchParam param) {
        ...
        //1.1 bool-must  按照模糊匹配
        if(!StringUtils.isEmpty(param.getKeyword())){
            boolQuery.must(QueryBuilders.matchQuery("skuTitle",param.getKeyword()));
        }
        //1.1 bool-filter  按照三级分类
